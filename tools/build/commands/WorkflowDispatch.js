"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
const chalk_1 = __importDefault(require("chalk"));
const inquirer_1 = __importDefault(require("inquirer"));
const open_1 = __importDefault(require("open"));
const Git_1 = __importDefault(require("../Git"));
const GitHubActions_1 = require("../GitHubActions");
const Utils_1 = require("../Utils");
const Logger_1 = __importDefault(require("../Logger"));
// Object containing configs for custom workflows.
// Custom workflows extends common workflows by providing specific inputs.
const CUSTOM_WORKFLOWS = {
    'ad-hoc-client-shell-app-ios-upload': {
        name: 'Ad-hoc iOS Client Shell App (with Upload to S3)',
        baseWorkflowSlug: 'ad-hoc-client-shell-app-ios',
        inputs: {
            upload: 'upload',
        },
    },
    'client-android-release': {
        name: 'Android Client Release',
        baseWorkflowSlug: 'client-android',
        inputs: {
            releaseGooglePlay: 'release-google-play',
        },
    },
    'client-android-simulator': {
        name: 'Android Client Simulator Release',
        baseWorkflowSlug: 'client-android',
        inputs: {
            releaseAPK: 'release-apk',
        },
    },
    'client-ios-simulator': {
        name: 'iOS Client Simulator Release',
        baseWorkflowSlug: 'client-ios',
        inputs: {
            releaseSimulator: 'release-simulator',
        },
    },
    'shell-app-ios-upload': {
        name: 'iOS Shell App (with Upload to S3)',
        baseWorkflowSlug: 'shell-app-ios',
        inputs: {
            upload: 'upload',
        },
    },
    'sdk-all': {
        name: 'SDK All',
        baseWorkflowSlug: 'sdk',
        inputs: {
            checkAll: 'check-all',
        },
    },
};
exports.default = (program) => {
    program
        .command('workflow-dispatch [workflowSlug]')
        .alias('dispatch', 'wd')
        .option('-r, --ref <ref>', 'The reference of the workflow run. The reference can be a branch, tag, or a commit SHA.')
        .option('--no-open', "Whether not to automatically open a page with workflow's job run containing the one that has just been triggered.", false)
        .description(`Dispatches an event that triggers a workflow on GitHub Actions. Requires ${chalk_1.default.magenta('GITHUB_TOKEN')} env variable to be set.`)
        .asyncAction(main);
};
/**
 * Main action of the command.
 */
async function main(workflowSlug, options) {
    if (!process.env.GITHUB_TOKEN) {
        throw new Error('Environment variable `GITHUB_TOKEN` must be set.');
    }
    const workflows = await getAllWorkflowsAsync();
    const workflow = await findWorkflowAsync(workflows, workflowSlug);
    const ref = options.ref || (await Git_1.default.getCurrentBranchNameAsync());
    if (!workflow) {
        throw new Error(`Unable to find workflow with slug \`${workflowSlug}\`.`);
    }
    // We need a confirmation to trigger a custom workflow.
    if (!process.env.CI && workflow.inputs && !(await confirmTriggeringWorkflowAsync(workflow))) {
        Logger_1.default.warn(`\n⚠️  Triggering custom workflow ${chalk_1.default.green(workflow.slug)} has been canceled.`);
        return;
    }
    // Get previously dispatched workflow run.
    const previousWorkflowRun = await GitHubActions_1.getLatestDispatchedWorkflowRunAsync(workflow.id);
    // Dispatch `workflow_dispatch` event.
    await GitHubActions_1.dispatchWorkflowEventAsync(workflow.id, ref, workflow.inputs);
    Logger_1.default.success('🎉 Successfully dispatched workflow event ');
    // Let's wait a little bit for the new workflow run to start and appear in the API response.
    Logger_1.default.info('⏳ Waiting for the new workflow run to start...');
    const newWorkflowRun = await Utils_1.retryAsync(2000, 10, async () => {
        const run = await GitHubActions_1.getLatestDispatchedWorkflowRunAsync(workflow.id);
        // Compare the result with previous workflow run.
        return (previousWorkflowRun === null || previousWorkflowRun === void 0 ? void 0 : previousWorkflowRun.id) !== (run === null || run === void 0 ? void 0 : run.id) ? run : undefined;
    });
    // Get a list of jobs for the new workflow run.
    const jobs = newWorkflowRun && (await GitHubActions_1.getJobsForWorkflowRunAsync(newWorkflowRun.id));
    // If the job exists, open it in web browser or print the link.
    if (jobs === null || jobs === void 0 ? void 0 : jobs[0]) {
        const url = jobs[0].html_url;
        if (options.open && !process.env.CI) {
            await open_1.default(url);
        }
        Logger_1.default.log(`🧭 You can open ${chalk_1.default.magenta(url)} to track the new workflow run.`);
    }
    else {
        Logger_1.default.warn(`⚠️  Cannot find any triggered jobs for ${chalk_1.default.green(workflow.slug)} workflow`);
    }
}
/**
 * Resolves to an array of workflows containing workflows fetched from the API
 * concatenated with custom workflows that declares some specific inputs.
 */
async function getAllWorkflowsAsync() {
    // Fetch workflows from GitHub Actions API.
    const commonWorkflows = await GitHubActions_1.getWorkflowsAsync();
    // Map custom workflow configs to workflows.
    const customWorkflows = Object.entries(CUSTOM_WORKFLOWS)
        .map(([customWorkflowSlug, workflowConfig]) => {
        const baseWorkflow = commonWorkflows.find((workflow) => workflow.slug === workflowConfig.baseWorkflowSlug);
        return baseWorkflow
            ? {
                ...Utils_1.deepCloneObject(baseWorkflow),
                name: workflowConfig.name,
                slug: customWorkflowSlug,
                baseSlug: workflowConfig.baseWorkflowSlug,
                inputs: workflowConfig.inputs,
            }
            : null;
    })
        .filter(Boolean);
    const allWorkflows = [].concat(commonWorkflows, customWorkflows);
    return allWorkflows.sort((a, b) => a.name.localeCompare(b.name));
}
/**
 * Finds workflow ID based on given name or config filename.
 */
async function findWorkflowAsync(workflows, workflowSlug) {
    var _a;
    if (!workflowSlug) {
        if (process.env.CI) {
            throw new Error('Command requires `workflowName` argument when run on the CI.');
        }
        return await promptWorkflowAsync(workflows);
    }
    return (_a = workflows.find((workflow) => workflow.slug === workflowSlug)) !== null && _a !== void 0 ? _a : null;
}
/**
 * Prompts for the workflow to trigger.
 */
async function promptWorkflowAsync(workflows) {
    const { workflow } = await inquirer_1.default.prompt([
        {
            type: 'list',
            name: 'workflow',
            message: 'Which workflow do you want to dispatch?',
            choices: workflows.map((workflow) => {
                return {
                    name: `${chalk_1.default.yellow(workflow.name)} (${chalk_1.default.green.italic(workflow.slug)})`,
                    value: workflow,
                };
            }),
            pageSize: workflows.length,
        },
    ]);
    return workflow;
}
/**
 * Requires the user to confirm dispatching an event that trigger given workflow.
 */
async function confirmTriggeringWorkflowAsync(workflow) {
    Logger_1.default.info(`\n👉 I'll trigger ${chalk_1.default.green(workflow.baseSlug)} workflow extended by the following input:`);
    Logger_1.default.log(workflow.inputs, '\n');
    const { confirm } = await inquirer_1.default.prompt([
        {
            type: 'confirm',
            name: 'confirm',
            message: 'Please type `y` and press enter if you want to continue',
            default: false,
        },
    ]);
    return confirm;
}
//# sourceMappingURL=WorkflowDispatch.js.map