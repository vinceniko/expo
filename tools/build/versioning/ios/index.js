...
exports.removeVersionAsync = exports.reinstallPodsAsync = exports.addVersionAsync = exports.versionReactNativeIOSFilesAsync = exports.versionVendoredModulesAsync = void 0;
...
const injectMacros_1 = require("./transforms/injectMacros");
const postTransforms_1 = require("./transforms/postTransforms");
const kernelFilesTransforms_1 = require("./transforms/kernelFilesTransforms");
const podspecTransforms_1 = require("./transforms/podspecTransforms");
const Packages_1 = require("../../Packages");
const Constants_1 = require("../../Constants");
var versionVendoredModules_1 = require("./versionVendoredModules");
Object.defineProperty(exports, "versionVendoredModulesAsync", { enumerable: true, get: function () { return versionVendoredModules_1.versionVendoredModulesAsync; } });
...
async function namespaceReactNativeFilesAsync(filenames, versionPrefix, versionedPodNames) {
    ...
    const taskQueue = new cwait_1.TaskQueue(Promise, 4); // Transform up to 4 files simultaneously.
    ...
    const transformSingleFile = taskQueue.wrap(async (filename) => {
        ...
        const targetPath = path_1.default.join(dirname, `${versionPrefix}${basename}`);
        ...
        for (const rule of filteredTransformRules) {
            await spawn_async_1.default('sed', [rule.flags || '-i', '--', rule.pattern, filename]);
        }
        // Rename file to be prefixed.
        await fs_extra_1.default.move(filename, targetPath);
        ...
        await _transformFileContentsAsync(targetPath, async (fileString) => {
            ...
            fileString = fileString.replace(/#(include|import)\s+"((?:[^"\/]+\/)?)([^"]+\.h)"/g, (match, p1, p2, p3) => {
            ...
            const injectedMacrosOutput = await transforms_1.runTransformPipelineAsync({
                pipeline: injectMacros_1.injectMacros(versionPrefix),
                input: fileString,
                targetPath,
            });
            return await transforms_1.runTransformPipelineAsync({
                pipeline: postTransforms_1.postTransforms(versionPrefix),
...
async function transformReactNativeAsync(rnPath, versionName, versionedPodNames) {
    ...
    let filenames = [];
    await Promise.all(filenameQueries.map(async (query) => {
        let queryFilenames = (await glob_promise_1.default(query));
        if (queryFilenames) {
 ...
 */
async function versionReactNativeIOSFilesAsync(globQuery, versionNumber) {
    let filenames = await glob_promise_1.default(globQuery);
...
exports.versionReactNativeIOSFilesAsync = versionReactNativeIOSFilesAsync;
async function generateVersionedReactNativeAsync(versionName) {
    const versionedReactNativePath = getVersionedReactNativePath(versionName);
    await fs_extra_1.default.mkdirs(versionedReactNativePath);
    // Clone react native latest version
    console.log(`Copying files from ${chalk_1.default.magenta(RELATIVE_RN_PATH)} ...`);
    await fs_extra_1.default.copy(path_1.default.join(Constants_1.EXPO_DIR, RELATIVE_RN_PATH, 'React'), path_1.default.join(versionedReactNativePath, 'React'));
    await fs_extra_1.default.copy(path_1.default.join(Constants_1.EXPO_DIR, RELATIVE_RN_PATH, 'Libraries'), path_1.default.join(versionedReactNativePath, 'Libraries'));
    await fs_extra_1.default.copy(path_1.default.join(Constants_1.EXPO_DIR, RELATIVE_RN_PATH, 'React.podspec'), path_1.default.join(versionedReactNativePath, 'React.podspec'));
    await fs_extra_1.default.copy(path_1.default.join(Constants_1.EXPO_DIR, RELATIVE_RN_PATH, 'React-Core.podspec'), path_1.default.join(versionedReactNativePath, 'React-Core.podspec'));
    await fs_extra_1.default.copy(path_1.default.join(Constants_1.EXPO_DIR, RELATIVE_RN_PATH, 'ReactCommon', 'ReactCommon.podspec'), path_1.default.join(versionedReactNativePath, 'ReactCommon', 'ReactCommon.podspec'));
    await fs_extra_1.default.copy(path_1.default.join(Constants_1.EXPO_DIR, RELATIVE_RN_PATH, 'ReactCommon', 'React-Fabric.podspec'), path_1.default.join(versionedReactNativePath, 'ReactCommon', 'React-Fabric.podspec'));
    await fs_extra_1.default.copy(path_1.default.join(Constants_1.EXPO_DIR, RELATIVE_RN_PATH, 'package.json'), path_1.default.join(versionedReactNativePath, 'package.json'));
    console.log(`Removing unnecessary ${chalk_1.default.magenta('*.js')} files ...`);
    const jsFiles = (await glob_promise_1.default(path_1.default.join(versionedReactNativePath, '**', '*.js')));
    for (const jsFile of jsFiles) {
        await fs_extra_1.default.remove(jsFile);
    }
    console.log(`Copying cpp libraries from ${chalk_1.default.magenta(path_1.default.join(RELATIVE_RN_PATH, 'ReactCommon'))} ...`);
    const cppLibraries = getCppLibrariesToVersion();
    await fs_extra_1.default.mkdirs(path_1.default.join(versionedReactNativePath, 'ReactCommon'));
    for (const library of cppLibraries) {
        await fs_extra_1.default.copy(path_1.default.join(Constants_1.EXPO_DIR, RELATIVE_RN_PATH, 'ReactCommon', library.libName), path_1.default.join(versionedReactNativePath, 'ReactCommon', library.libName));
    }
 ...
 */
async function modifyKernelFilesAsync(versionName, rollback = false) {
    const kernelFilesPath = path_1.default.join(Constants_1.IOS_DIR, 'Exponent/kernel');
    ...
    await Promise.all(filenames.map(async (filename) => {
        console.log(`Modifying ${chalk_1.default.magenta(path_1.default.relative(Constants_1.EXPO_DIR, filename))}:`);
        await _transformFileContentsAsync(filename, (fileContents) => transforms_1.runTransformPipelineAsync({
            pipeline: kernelFilesTransforms_1.kernelFilesTransforms(versionName, rollback),
            targetPath: filename,
            input: fileContents,
        }));
    }));
 ...
 */
async function generateAutolinkingScriptAsync(versionedReactNativePath, versionName) {
    const targetAutolinkPath = path_1.default.join(versionedReactNativePath, 'react_native_pods.rb');
    await fs_extra_1.default.copy(path_1.default.join(Constants_1.EXPO_DIR, RELATIVE_RN_PATH, 'scripts', 'react_native_pods.rb'), targetAutolinkPath);
    const targetSource = (await fs_extra_1.default.readFile(targetAutolinkPath, 'utf8'))
    ...
    await fs_extra_1.default.writeFile(targetAutolinkPath, targetSource);
}
async function generateReactNativePodspecsAsync(versionedReactNativePath, versionName) {
    const podspecFiles = await glob_promise_1.default(path_1.default.join(versionedReactNativePath, '**', '*.podspec'));
    for (const podspecFile of podspecFiles) {
        const basename = path_1.default.basename(podspecFile, '.podspec');
        if (/^react$/i.test(basename)) {
            continue;
        }
        console.log(`Generating podspec for ${chalk_1.default.green(basename)} at ${chalk_1.default.magenta(path_1.default.relative(versionedReactNativePath, podspecFile))} ...`);
        const podspecSource = await fs_extra_1.default.readFile(podspecFile, 'utf8');
        const podspecOutput = await transforms_1.runTransformPipelineAsync({
            pipeline: podspecTransforms_1.podspecTransforms(versionName),
            input: podspecSource,
            targetPath: podspecFile,
        });
        // Write transformed podspec output to the prefixed file.
        await fs_extra_1.default.writeFile(path_1.default.join(path_1.default.dirname(podspecFile), `${versionName}${basename}.podspec`), podspecOutput);
        // Remove original and unprefixed podspec.
        await fs_extra_1.default.remove(podspecFile);
    }
 ...
 */
async function generateVersionedExpoAsync(versionName, versionNumber) {
    ...
    await fs_extra_1.default.mkdirs(versionedExpoKitPath);
    // Copy versioned exponent modules into the clone
    console.log(`Copying versioned native modules into the new Pod...`);
    await fs_extra_1.default.copy(path_1.default.join(Constants_1.IOS_DIR, 'Exponent', 'Versioned'), versionedExpoKitPath);
    await fs_extra_1.default.copy(path_1.default.join(Constants_1.EXPO_DIR, 'ios', 'ExpoKit.podspec'), path_1.default.join(versionedExpoKitPath, 'ExpoKit.podspec'));
    ...
    for (const pkg of packages) {
        const modulePath = path_1.default.join(Constants_1.EXPO_DIR, RELATIVE_UNIVERSAL_MODULES_PATH, pkg.packageName);
        const podName = pkg.podspecName;
        if (podName && pkg.isVersionableOnPlatform('ios') && !excludedPodNames.includes(podName)) {
            await fs_extra_1.default.copy(path_1.default.join(modulePath, 'ios'), path_1.default.join(versionedExpoPath, podName));
            await fs_extra_1.default.move(path_1.default.join(versionedExpoPath, podName, podName), path_1.default.join(versionedExpoPath, podName, versionedUnimodulePods[podName]));
            await fs_extra_1.default.copy(path_1.default.join(modulePath, 'package.json'), path_1.default.join(versionedExpoPath, podName, 'package.json'));
        }
    }
    for (const originalUnimodulePodName of originalUnimodulePodNames) {
        ...
        const originalPodSpecPath = path_1.default.join(versionedExpoPath, originalUnimodulePodName, `${originalUnimodulePodName}.podspec`);
        const prefixedPodSpecPath = path_1.default.join(versionedExpoPath, originalUnimodulePodName, `${versionedUnimodulePodName}.podspec`);
        if (!(await fs_extra_1.default.pathExists(originalPodSpecPath))) {
            continue;
        }
        console.log(`Generating podspec for ${chalk_1.default.green(originalUnimodulePodName)} ...`);
        await fs_extra_1.default.move(originalPodSpecPath, prefixedPodSpecPath);
        ...
        await spawn_async_1.default('sed', [
            '-Ei',
            '--',
    ...
    console.log(`Generating podspec for ${chalk_1.default.green('ExpoKit')} ...`);
 ...
 */
async function generateExpoKitPodspecAsync(specfilePath, universalModulesPodNames, versionName, versionNumber) {
    const versionedReactPodName = getVersionedReactPodName(versionName);
    const versionedExpoKitPodName = getVersionedExpoKitPodName(versionName);
    const specFilename = path_1.default.join(specfilePath, 'ExpoKit.podspec');
    const excludedPodNames = getExcludedPodNames();
    // rename spec to newPodName
    const sedPattern = `s/\\(s\\.name[[:space:]]*=[[:space:]]\\)"ExpoKit"/\\1"${versionedExpoKitPodName}"/g`;
    await spawn_async_1.default('sed', ['-i', '--', sedPattern, specFilename]);
    ...
    await _transformFileContentsAsync(specFilename, async (fileString) => {
        ...
        const universalModulesDependencies = (await Packages_1.getListOfPackagesAsync())
            .filter((pkg) => pkg.isIncludedInExpoClientOnPlatform('ios') &&
            pkg.podspecName &&
            !excludedPodNames.includes(pkg.podspecName))
            .map(({ podspecName }) => `ss.dependency         "${universalModulesPodNames[podspecName] || podspecName}"`).join(`
    `);
        const externalDependencies = EXTERNAL_REACT_ABI_DEPENDENCIES.map((podName) => `ss.dependency         "${podName}"`).join(`
    `);
  ...
  s.subspec "ExpoOptional" do |ss|
    ...
    ss.source_files     = "Optional/**/*.{h,m,mm}"
        ...
        fileString = fileString.replace(/(s\.subspec ".+?"[\S\s]+?(?=end\b)end\b[\s]+)+/g, `${subspec}\n`);
...
boost_compiler_flags = '-Wno-documentation'\n\n`)
  ...
  s.xcconfig               = { 
    ...
    await fs_extra_1.default.move(specFilename, path_1.default.join(specfilePath, `${versionedExpoKitPodName}.podspec`));
    return;
}
/**
...
async function generateReactPodspecAsync(versionedReactNativePath, versionName) {
    ...
    const specFilename = path_1.default.join(versionedReactNativePath, 'React.podspec');
    // rename spec to newPodName
    ...
    await _transformFileContentsAsync(specFilename, (fileString) => {
        ...
        fileString = fileString.replace(/(\.dependency\s+)"React([^"]+)"/g, `$1"${versionedReactPodName}$2"`);
        fileString = fileString.replace('/RCTTV', `/${versionName}RCTTV`);
        // namespace cpp libraries
        const cppLibraries = getCppLibrariesToVersion();
        cppLibraries.forEach(({ libName }) => {
            fileString = fileString.replace(new RegExp(`([^A-Za-z0-9_])${libName}([^A-Za-z0-9_])`, 'g'), `$1${getVersionedLibraryName(libName, versionName)}$2`);
        });
        // fix wrong Yoga pod name
        fileString = fileString.replace(/^(.*dependency.*["']).*yoga.*?(["'].*)$/m, `$1${versionedYogaPodName}$2`);
        return fileString;
    });
    // move podspec to ${versionedReactPodName}.podspec
    await fs_extra_1.default.move(specFilename, path_1.default.join(versionedReactNativePath, `${versionedReactPodName}.podspec`));
    return;
}
 ...
 */
async function generatePodfileSubscriptsAsync(versionNumber, versionName, versionedPodNames, versionedReactPodPath) {
    if (!versionedPodNames.React) {
        throw new Error('Tried to add generate pod dependencies, but missing a name for the versioned library.');
    }
    const relativeReactNativePath = path_1.default.relative(Constants_1.IOS_DIR, getVersionedReactNativePath(versionName));
    const relativeExpoKitPath = path_1.default.relative(Constants_1.IOS_DIR, getVersionedExpoKitPath(versionName));
    const relativeExpoPath = path_1.default.relative(Constants_1.IOS_DIR, getVersionedExpoPath(versionName));
...
use_pods! 'vendored/sdk${semver_1.default.major(versionNumber)}/*/*.podspec.json', '${versionName}'

${versionableUnimodulesPods}
`;
    await fs_extra_1.default.writeFile(path_1.default.join(versionedReactPodPath, 'dependencies.rb'), dep);
    // Add postinstall.
    ...
    let configValues = getCFlagsToPrefixGlobals(versionedPodNames.React, globals.React.concat(globals.yoga));
...
`;
    await fs_extra_1.default.writeFile(path_1.default.join(versionedReactPodPath, 'postinstalls.rb'), config);
}
/**
...
async function modifyVersionConfigAsync(configPath, transformConfig) {
    ...
    await spawn_async_1.default('plutil', [
        '-convert',
        'xml1',
        jsConfigFilename,
        '-o',
        path_1.default.join(configPath, 'EXSDKVersions.plist'),
    ]);
    return;
}
function validateAddVersionDirectories(rootPath, newVersionPath) {
    ...
    relativePathsToCheck.forEach((path) => {
        try {
            fs_extra_1.default.accessSync(`${rootPath}/${path}`, fs_extra_1.default.constants.F_OK);
        }
        catch (e) {
    ...
    try {
        ...
        fs_extra_1.default.accessSync(newVersionPath, fs_extra_1.default.constants.F_OK);
...
function validateRemoveVersionDirectories(rootPath, newVersionPath) {
    ...
    pathsToCheck.forEach((path) => {
        try {
            fs_extra_1.default.accessSync(path, fs_extra_1.default.constants.F_OK);
        }
        catch (e) {
...
async function getConfigsFromArguments(versionNumber) {
    ...
    let rootPathComponents = Constants_1.EXPO_DIR.split('/');
    let versionPathComponents = path_1.default.join('ios', 'versioned-react-native', versionName).split('/');
...
async function getVersionedUnimodulePodsAsync(versionName) {
...
function getVersionedReactPodName(versionName) {
    return getVersionedLibraryName('React', versionName);
}
function getVersionedYogaPodName(versionName) {
    return getVersionedLibraryName('Yoga', versionName);
}
function getVersionedJSIPodName(versionName) {
    return getVersionedLibraryName('jsiReact', versionName);
}
function getVersionedExpoKitPodName(versionName) {
    return getVersionedLibraryName('ExpoKit', versionName);
}
function getVersionedLibraryName(libraryName, versionName) {
    return `${versionName}${libraryName}`;
}
function getVersionedReactNativePath(versionName) {
    return path_1.default.join(Constants_1.VERSIONED_RN_IOS_DIR, versionName, 'ReactNative');
}
function getVersionedExpoPath(versionName) {
    return path_1.default.join(Constants_1.VERSIONED_RN_IOS_DIR, versionName, 'Expo');
}
function getVersionedExpoKitPath(versionName) {
    return path_1.default.join(getVersionedExpoPath(versionName), 'ExpoKit');
}
...
async function addVersionAsync(versionNumber) {
    let { versionName, newVersionPath, versionedPodNames } = await getConfigsFromArguments(versionNumber);
    // Validate the directories we need before doing anything
    console.log(`Validating root directory ${chalk_1.default.magenta(Constants_1.EXPO_DIR)} ...`);
    let isFilesystemReady = validateAddVersionDirectories(Constants_1.EXPO_DIR, newVersionPath);
    ...
    console.log(`Creating new ABI version ${chalk_1.default.cyan(versionNumber)} at ${chalk_1.default.magenta(path_1.default.relative(Constants_1.EXPO_DIR, newVersionPath))}`);
    await fs_extra_1.default.mkdirs(newVersionPath);
    ...
    console.log(`Generating ${chalk_1.default.magenta(path_1.default.relative(Constants_1.EXPO_DIR, getVersionedExpoPath(versionName)))} directory...`);
    ...
    await generatePodfileSubscriptsAsync(versionNumber, versionName, versionedPodNames, newVersionPath);
    ...
    await modifyVersionConfigAsync(path_1.default.join(Constants_1.IOS_DIR, 'Exponent', 'Supporting'), (config) => addVersionToConfig(config, versionNumber));
    await modifyVersionConfigAsync(path_1.default.join(Constants_1.EXPO_DIR, 'exponent-view-template', 'ios', 'exponent-view-template', 'Supporting'), (config) => addVersionToConfig(config, versionNumber));
    // Modifying kernel files
    console.log(`Modifying ${chalk_1.default.bold('kernel files')} to incorporate new SDK version...`);
    await modifyKernelFilesAsync(versionName);
    console.log('Removing any `filename--` files from the new pod ...');
    try {
        const minusMinusFiles = await glob_promise_1.default(path_1.default.join(newVersionPath, '**', '*--'));
        for (const minusMinusFile of minusMinusFiles) {
            await fs_extra_1.default.remove(minusMinusFile);
        }
    }
    catch (error) {
...
async function askToReinstallPodsAsync() {
    if (process.env.CI) {
...
async function reinstallPodsAsync(force) {
    if (force || (force !== false && (await askToReinstallPodsAsync()))) {
        await spawn_async_1.default('pod', ['install'], { stdio: 'inherit', cwd: Constants_1.IOS_DIR });
...
exports.reinstallPodsAsync = reinstallPodsAsync;
async function removeVersionAsync(versionNumber) {
    let { newVersionPath, versionedPodNames, versionName } = await getConfigsFromArguments(versionNumber);
    console.log(`Removing SDK version ${chalk_1.default.cyan(versionNumber)} from ${chalk_1.default.magenta(path_1.default.relative(Constants_1.EXPO_DIR, newVersionPath))} with Pod name ${chalk_1.default.green(versionedPodNames.React)}`);
    // Validate the directories we need before doing anything
    console.log(`Validating root directory ${chalk_1.default.magenta(Constants_1.EXPO_DIR)} ...`);
    let isFilesystemReady = validateRemoveVersionDirectories(Constants_1.EXPO_DIR, newVersionPath);
    if (!isFilesystemReady) {
        console.log('Aborting: At least one directory we expect is not available');
        return;
    }
    // remove directory
    console.log(`Removing versioned files under ${chalk_1.default.magenta(path_1.default.relative(Constants_1.EXPO_DIR, newVersionPath))}...`);
    await fs_extra_1.default.remove(newVersionPath);
    // remove dep from main podfile
    console.log(`Removing ${chalk_1.default.green(versionedPodNames.React)} dependency from root Podfile...`);
    ...
    await modifyVersionConfigAsync(path_1.default.join(Constants_1.IOS_DIR, 'Exponent', 'Supporting'), (config) => removeVersionFromConfig(config, versionNumber));
    await modifyVersionConfigAsync(path_1.default.join(Constants_1.EXPO_DIR, 'exponent-view-template', 'ios', 'exponent-view-template', 'Supporting'), (config) => removeVersionFromConfig(config, versionNumber));
    // modify kernel files
...
async function _transformFileContentsAsync(filename, transform) {
    let fileString = await fs_extra_1.default.readFile(filename, 'utf8');
    let newFileString = await transform(fileString);
    if (newFileString !== null) {
        await fs_extra_1.default.writeFile(filename, newFileString);
    }
    return;
}
