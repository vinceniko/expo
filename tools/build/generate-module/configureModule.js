...
const renameFilesWithExtensions = async (directoryPath, extensions, renamings) => {
    await asyncForEach(renamings, async ({ from, to }) => await asyncForEach(extensions, async (extension) => {
        const fromFilename = `${from}${extension}`;
        if (!fs.existsSync(path.join(directoryPath, fromFilename))) {
            return;
        }
        const toFilename = `${to}${extension}`;
        await fs.rename(path.join(directoryPath, fromFilename), path.join(directoryPath, toFilename));
    }));
};
/**
...
const replaceContents = async (directoryPath, replaceFunction) => {
    await Promise.all(klaw_sync_1.default(directoryPath, { nodir: true }).map((file) => replaceContent(file.path, replaceFunction)));
};
/**
 ...
 */
async function configureIOS(modulePath, { podName, jsPackageName, viewManager }) {
    ...
    await renameFilesWithExtensions(path.join(iosPath, 'EXModuleTemplate'), ['.h', '.m'], [
    ...
    await renameFilesWithExtensions(iosPath, ['', '.podspec'], [{ from: 'EXModuleTemplate', to: `${podName}` }]);
    await replaceContents(iosPath, (singleFileContent) => singleFileContent
...
function findAndroidSourceDir(androidPath, flavor = 'main') {
    ...
    if (!javaExists && !kotlinExists) {
        throw new Error(`Invalid template. Android source directory not found: ${androidSrcPathBase}/[java|kotlin]`);
    }
...
function findTemplateAndroidPackage(flavorSrcPath) {
    const srcFiles = klaw_sync_1.default(flavorSrcPath, {
 ...
 * @param configuration - naming configuration
 */
async function configureAndroid(modulePath, { javaPackage, jsPackageName, viewManager }) {
    ...
    if (!viewManager) {
        ...
        replaceContent(path.join(sourceFilesPath, 'ModuleTemplatePackage.kt'), (packageContent) => packageContent
    ...
    await removeUponEmptyOrOnlyEmptySubdirs(path.join(androidSrcPath, templateJavaPackage.split('.')[0]));
    // prepare tests
    if (fs.existsSync(path.resolve(androidPath, 'src', 'test'))) {
        ...
        await removeUponEmptyOrOnlyEmptySubdirs(path.join(androidTestPath, templateTestPackage.split('.')[0]));
        ...
        await renameFilesWithExtensions(testDestinationPath, ['.kt', '.java'], [{ from: 'ModuleTemplateModuleTest', to: `${moduleName}ModuleTest` }]);
    }
    // Replace contents of destination files
    await replaceContents(androidPath, (singleFileContent) => singleFileContent
        .replace(new RegExp(templateJavaPackage, 'g'), javaPackage)
        .replace(/ModuleTemplate/g, moduleName)
        .replace(/ExpoModuleTemplate/g, jsPackageName));
    await replaceContent(path.join(androidPath, 'build.gradle'), (gradleContent) => gradleContent
 ...
 * @param configuration - naming configuration
 */
async function configureTS(modulePath, { jsPackageName, viewManager }) {
    ...
    if (!viewManager) {
        ...
        await replaceContent(path.join(tsPath, 'ModuleTemplate.ts'), (fileContent) => fileContent.replace(/(^\s+)+(^.*?){1}ExpoModuleTemplateView.*$/m, ''));
    }
    await renameFilesWithExtensions(path.join(tsPath, '__tests__'), ['.ts'], [{ from: 'ModuleTemplate-test', to: `${moduleName}-test` }]);
    ...
    await replaceContents(tsPath, (singleFileContent) => singleFileContent
 ...
 * @param configuration - naming configuration
 */
async function configureNPM(modulePath, { npmModuleName, podName, jsPackageName }) {
    const [, moduleName] = preparePrefixes(jsPackageName);
    await replaceContent(path.join(modulePath, 'package.json'), (singleFileContent) => singleFileContent
        .replace(/expo-module-template/g, npmModuleName)
        .replace(/"version": "[\w.-]+"/, '"version": "1.0.0"')
        .replace(/ExpoModuleTemplate/g, jsPackageName)
        .replace(/ModuleTemplate/g, moduleName));
    await replaceContent(path.join(modulePath, 'README.md'), (readmeContent) => readmeContent
        .replace(/expo-module-template/g, npmModuleName)
        .replace(/ExpoModuleTemplate/g, jsPackageName)
        .replace(/EXModuleTemplate/g, podName));
}
