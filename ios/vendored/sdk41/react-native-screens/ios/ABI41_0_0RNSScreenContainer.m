...
@interface ABI41_0_0RNSScreenContainerManager : ABI41_0_0RCTViewManager

- (void)markUpdated:(ABI41_0_0RNSScreenContainerView *)screen;

@end

@interface ABI41_0_0RNSScreenContainerView () <ABI41_0_0RCTInvalidating>

@property (nonatomic, retain) UIViewController *controller;
@property (nonatomic, retain) NSMutableSet<ABI41_0_0RNSScreenView *> *activeScreens;
@property (nonatomic, retain) NSMutableArray<ABI41_0_0RNSScreenView *> *ABI41_0_0ReactSubviews;

- (void)updateContainer;

@end

@implementation ABI41_0_0RNScreensViewController
...
- (UIViewController *)findActiveChildVC
{
  for (UIViewController *childVC in self.childViewControllers) {
    if ([childVC isKindOfClass:[ABI41_0_0RNSScreen class]] && ((ABI41_0_0RNSScreenView *)((ABI41_0_0RNSScreen *)childVC.view)).activityState == ABI41_0_0RNSActivityStateOnTop) {
      return childVC;
    }
  }
...
@implementation ABI41_0_0RNSScreenContainerView {
  BOOL _needUpdate;
  BOOL _invalidated;
  __weak ABI41_0_0RNSScreenContainerManager *_manager;
}

- (instancetype)initWithManager:(ABI41_0_0RNSScreenContainerManager *)manager
{
  if (self = [super init]) {
    _activeScreens = [NSMutableSet new];
    _ABI41_0_0ReactSubviews = [NSMutableArray new];
    _controller = [[ABI41_0_0RNScreensViewController alloc] init];
    _needUpdate = NO;
    _invalidated = NO;
    _manager = manager;
...
- (void)insertReactSubview:(ABI41_0_0RNSScreenView *)subview atIndex:(NSInteger)atIndex
{
  ...
  [subview ABI41_0_0ReactSetFrame:CGRectMake(0, 0, self.bounds.size.width, self.bounds.size.height)];
}
...
- (UIViewController*)findChildControllerForScreen:(ABI41_0_0RNSScreenView*)screen
{
...
- (void)prepareDetach:(ABI41_0_0RNSScreenView *)screen
{
  [[self findChildControllerForScreen:screen] willMoveToParentViewController:nil];
}

- (void)detachScreen:(ABI41_0_0RNSScreenView *)screen
{
...
- (void)attachScreen:(ABI41_0_0RNSScreenView *)screen atIndex:(NSInteger)index
{
...
- (void)updateContainer
{
  ...
  for (ABI41_0_0RNSScreenView *screen in _ABI41_0_0ReactSubviews) {
    if (screen.activityState == ABI41_0_0RNSActivityStateInactive && [_activeScreens containsObject:screen]) {
      screenRemoved = YES;
      [self detachScreen:screen];
    }
    [orphaned removeObject:screen];
  }
  for (ABI41_0_0RNSScreenView *screen in orphaned) {
    screenRemoved = YES;
    [self detachScreen:screen];
  }
  ...
  if (screenAdded) {
    ...
    NSInteger index = 0;
    for (ABI41_0_0RNSScreenView *screen in _ABI41_0_0ReactSubviews) {
      if (screen.activityState != ABI41_0_0RNSActivityStateInactive) {
        if ([_activeScreens containsObject:screen] && screen.activityState == ABI41_0_0RNSActivityStateTransitioningOrBelowTop) {
  ...
  for (ABI41_0_0RNSScreenView *screen in _ABI41_0_0ReactSubviews) {
    if (screen.activityState == ABI41_0_0RNSActivityStateOnTop) {
...
- (void)layoutSubviews
{
  ...
  for (ABI41_0_0RNSScreenView *subview in _ABI41_0_0ReactSubviews) {
    [subview ABI41_0_0ReactSetFrame:CGRectMake(0, 0, self.bounds.size.width, self.bounds.size.height)];
...
@implementation ABI41_0_0RNSScreenContainerManager {
  NSMutableArray<ABI41_0_0RNSScreenContainerView *> *_markedContainers;
}
...
- (UIView *)view
{
  ...
  return [[ABI41_0_0RNSScreenContainerView alloc] initWithManager:self];
}
...
- (void)markUpdated:(ABI41_0_0RNSScreenContainerView *)screen
{
  ...
  if ([_markedContainers count] == 1) {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
      for (ABI41_0_0RNSScreenContainerView *container in self->_markedContainers) {
