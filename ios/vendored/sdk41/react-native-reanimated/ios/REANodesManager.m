...
// Interface below has been added in order to use private methods of ABI41_0_0RCTUIManager,
// ABI41_0_0RCTUIManager#UpdateView is a ABI41_0_0React Method which is exported to JS but in
// Objective-C it stays private
// ABI41_0_0RCTUIManager#setNeedsLayout is a method which updated layout only which
...
@interface ABI41_0_0RCTUIManager (SyncUpdates)

- (BOOL)hasEnqueuedUICommands;

- (void)runSyncUIUpdatesWithObserver:(id<ABI41_0_0RCTUIManagerObserver>)observer;

@end

@implementation ABI41_0_0RCTUIManager (SyncUpdates)

- (BOOL)hasEnqueuedUICommands
{
  // Accessing some private bits of ABI41_0_0RCTUIManager to provide missing functionality
...
- (void)runSyncUIUpdatesWithObserver:(id<ABI41_0_0RCTUIManagerObserver>)observer
{
  ...
  NSHashTable<id<ABI41_0_0RCTUIManagerObserver>> *soleObserver = [NSHashTable new];
...
@interface REANodesManager() <ABI41_0_0RCTUIManagerObserver>
...
@implementation REANodesManager
{
  ...
  NSMutableArray<id<ABI41_0_0RCTEvent>> *_eventQueue;
...
- (instancetype)initWithModule:(REAModule *)reanimatedModule
                     uiManager:(ABI41_0_0RCTUIManager *)uiManager
{
  if ((self = [super init])) {
...
- (void)onAnimationFrame:(CADisplayLink *)displayLink
{
  ...
  for (NSUInteger i = 0; i < _eventQueue.count; i++) {
    id<ABI41_0_0RCTEvent> event = _eventQueue[i];
...
- (BOOL)uiManager:(ABI41_0_0RCTUIManager *)manager performMountingWithBlock:(ABI41_0_0RCTUIManagerMountingBlock)block {
  ABI41_0_0RCTAssert(_mounting == nil, @"Mouting block is expected to not be set");
  _mounting = block;
  return YES;
}
...
- (void)enqueueUpdateViewOnNativeThread:(nonnull NSNumber *)reactTag
  ...
  [_operationsInBatch addObject:^(ABI41_0_0RCTUIManager *uiManager) {
...
{
  ABI41_0_0RCTAssertParam(parentID);
  ABI41_0_0RCTAssertParam(childID);
...
- (void)connectNodeToView:(REANodeID)nodeID
                 ...
                 viewName:(NSString *)viewName
{
  ABI41_0_0RCTAssertParam(nodeID);
  REANode *node = _nodes[nodeID];
  ABI41_0_0RCTAssertParam(node);
...
{
  ABI41_0_0RCTAssertParam(eventNodeID);
  REANode *eventNode = _nodes[eventNodeID];
  ABI41_0_0RCTAssert([eventNode isKindOfClass:[REAEventNode class]], @"Event node is of an invalid type");

  NSString *key = [NSString stringWithFormat:@"%@%@",
                   viewTag,
                   ABI41_0_0RCTNormalizeInputEventName(eventName)];
  ABI41_0_0RCTAssert([_eventMapping objectForKey:key] == nil, @"Event handler already set for the given view and event type");
...
- (void)processEvent:(id<ABI41_0_0RCTEvent>)event
{
  NSString *key = [NSString stringWithFormat:@"%@%@",
                   event.viewTag,
                   ABI41_0_0RCTNormalizeInputEventName(event.eventName)];
  REAEventNode *eventNode = [_eventMapping objectForKey:key];
  [eventNode processEvent:event];
}

- (void)processDirectEvent:(id<ABI41_0_0RCTEvent>)event
{
...
- (BOOL)isDirectEvent:(id<ABI41_0_0RCTEvent>)event
{
  ...
  return [directEventNames containsObject:ABI41_0_0RCTNormalizeInputEventName(event.eventName)];
}
...
- (void)dispatchEvent:(id<ABI41_0_0RCTEvent>)event
{
  ...
  if (_eventHandler != nil) {
    ...
    __weak typeof(self) weakSelf = self;
    ABI41_0_0RCTExecuteOnMainQueue(^void(){
      __typeof__(self) strongSelf = weakSelf;
      if (strongSelf == nil) {
        return;
      }
